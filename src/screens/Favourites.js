...
export default function Favourites() {
  ...
  // ─── Load favourites on focus ─────────────────────────────────────────────

  useFocusEffect(
    useCallback(() => {
      session('favourites').then((response) => {
        if (response) setFavouriteIds(JSON.parse(response))
        else setFavouriteIds([])
      })
    }, [])
  )
  ...
  const filteredData = useMemo(() => {
    ...
    return favouriteRecipes.filter(item => {
  ...
  }, [favouriteRecipes, searchInput, selectedCategory, vegetarianOnly, selectedRating, selectedTime])
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.whiteMain }}>
      <CollapsibleHeader
        title={`Favourites (${filteredData.length})`}
      ...
      >
        ...
        {favouriteIds.length === 0 ? (
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingTop: 80 }}>
            <Ionicons name="heart-outline" size={48} color={COLORS.black900} style={{ opacity: 0.2 }} />
            <Text style={{ marginTop: 12, color: COLORS.black900, opacity: 0.4, fontSize: 15 }}>
              No favourites yet
            </Text>
            <Text style={{ marginTop: 4, color: COLORS.black900, opacity: 0.3, fontSize: 13 }}>
