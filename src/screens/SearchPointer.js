...
export default function SearchPointer() {
  ...
  // route.params?.data is the full list passed from previous screen
  const sourceData = useMemo(() => route.params?.data ?? [], [])
  const selectedIng = route.params?.selectedIng
  ...
  }, [searchInput, selectedCategory, vegetarianOnly, selectedRating, selectedTime, sourceData])
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.whiteMain }}>
      <CollapsibleHeader
        title={`${filteredData.length} Recipe${filteredData.length === 1 ? '' : 's'} Found`}
